...
include("functions/functions.php");
  if(isset($_POST['sign_up'])){
  	...
  	if ($check == 1) {
  	  ...
  	  $rand = rand(1,3);  //random no. b/w 1 and 3
  	  if($rand == 1){
  	  ...
  	  $insert = "insert into users (f_name,l_name,user_name, describe_user,Relationship, user_pass, user_email, user_country, user_gender, user_birthday , user_image , user_cover, user_reg_date, status , posts , recovery_account)
  	      ...
  	      values('$first_name' , '$last_name','$username', 'Hello everyone this is my default status', '...', '$pass' , '$email', '$country', '$gender', '$birthday' ,'$profile_pic' , 'default_cover.jpg' ,NOW() ,'$status' ,'$posts' ,'iwanttoputadingintheuniverse.')";
  		...
  		echo "<script>window.open('signin.php', '_self')</script>" ;
  	      	   ...
  	      	   echo "<script>alert('Registration failed, pleased try again!')</script>" ;
  		echo "<script>window.open('signup.php', '_self')</script>" ;
  	      }
  }
?>